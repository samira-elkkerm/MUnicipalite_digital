...
const ModifierGastronomie = ({ show, onHide, gastronomie, onGastronomieUpdated }) => {
  ...
  // Initialiser les données du formulaire avec la gastronomie à modifier
  useEffect(() => {
    if (gastronomie) {
      setFormData({
        plats: gastronomie.plats || '',
        image: null,
        cuisine: gastronomie.cuisine || '',
        description: gastronomie.description || '',
        currentImage: gastronomie.image || ''
      });
  ...
  const validateForm = () => {
    ...
    if (!formData.plats.trim()) newErrors.plats = "Le nom du plat est requis";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/gastronomies/${gastronomie.id}`, {
        method: 'POST',
        body: formDataToSend,
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.message || 'Erreur lors de la mise à jour du plat');
      }
      
      setSuccessMessage("Plat gastronomique mis à jour avec succès !");
      onGastronomieUpdated(data);
    ...
    >
      <Modal.Header closeButton className="border-0 pb-0">
        <Modal.Title className="w-100 text-center fw-bold fs-4">
          Modifier le plat gastronomique
        </Modal.Title>
      </Modal.Header>
...
export default ModifierGastronomie;